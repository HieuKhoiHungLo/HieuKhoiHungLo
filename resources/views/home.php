...
?> 
    ...
    <style>
        ...
        @media (max-width: 768px) {
            .marquee-content { animation-duration: 80s; }
            #live-clock-container { min-width: 140px; padding-left: 1rem; padding-right: 2rem; }
            #live-clock { min-width: 120px; font-size: 10px; }
        }
    </style>
    <?php endif; ?>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 -mt-10 relative z-20">
            ...
            <div class="p-8 md:p-12">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php foreach ($choices as $choice): ?>
                                <?php 
                                    ...
                                    $statusClass = $isPass ? 'bg-green-100 text-green-700 border-green-200' : 
                                        ((strpos($status, 'Truot') !== false) ? 'bg-gray-100 text-gray-500 border-gray-200' : 'bg-yellow-100 text-yellow-700 border-yellow-200');
                                ?>
                                <tr class="<?= $rowClass ?> hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <?php foreach ($majors as $m) { if ($m['ma_nganh'] == $choice['ma_nganh']) { echo '<div class="text-xs text-gray-500 mt-0.5">' . htmlspecialchars($m['ten_nganh']) . '</div>'; break; } } ?>
                                    </td>
                ...
                <div class="flex items-center justify-between mb-8">
                    ...
                    <div class="hidden md:block">
                        <div class="w-16 h-16 rounded-full bg-red-50 flex items-center justify-center text-hvu-red text-2xl font-black border-4 border-white shadow-lg">
                ...
                <div class="relative">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-<?= $totalSteps ?> gap-6 relative z-10">
                        <!-- Step 1 -->
                        <?php $s=1; $sDone=$isDone($s); $sActive=($nextStep==$s); ?>
                        <div class="group relative bg-white p-4 rounded-2xl border-2 <?= $sActive ? 'border-hvu-red shadow-lg shadow-red-100' : ($sDone ? 'border-green-500' : 'border-gray-100') ?> transition hover:-translate-y-1 text-center md:text-left">
                            ...
                            <a href="<?= url('/profile/step1') ?>" class="text-[10px] uppercase font-black tracking-widest mt-2 inline-block <?= $sActive ? 'text-hvu-red' : 'text-gray-400' ?>">
                                <?= $sDone ? 'Cập nhật' : ($sActive ? 'Thực hiện ngay' : 'Đang khóa') ?>
                            </a>
                        </div>

                        <!-- Step 2 -->
                        <?php $s=2; $sDone=$isDone($s); $sActive=($nextStep==$s); ?>
                        <div class="group relative bg-white p-4 rounded-2xl border-2 <?= $sActive ? 'border-hvu-red shadow-lg shadow-red-100' : ($sDone ? 'border-green-500' : 'border-gray-100') ?> transition hover:-translate-y-1 text-center md:text-left">
                           <div class="w-10 h-10 mx-auto md:mx-0 rounded-full flex items-center justify-center font-bold text-sm mb-3 <?= $sDone ? 'bg-green-500 text-white' : ($sActive ? 'bg-hvu-red text-white' : 'bg-gray-100 text-gray-400') ?>">
                                <?= $sDone ? '<i class="fas fa-check"></i>' : $s ?>
                            </div>
                        ...
                        <div class="group relative bg-white p-4 rounded-2xl border-2 <?= $sActive ? 'border-hvu-red shadow-lg shadow-red-100' : ($sDone ? 'border-green-500' : 'border-gray-100') ?> transition hover:-translate-y-1 text-center md:text-left">
                            ...
                            <h3 class="font-bold text-gray-900">Chứng chỉ QT</h3>
                        ...
                        <!-- Step 4 (Conditional) -->
                        <?php if($enableTHPT): ?>
                        <?php $s=4; $sDone=$isDone($s); $sActive=($nextStep==$s); ?>
                        <div class="group relative bg-white p-4 rounded-2xl border-2 <?= $sActive ? 'border-hvu-red shadow-lg shadow-red-100' : ($sDone ? 'border-green-500' : 'border-gray-100') ?> transition hover:-translate-y-1 text-center md:text-left">
                             <div class="w-10 h-10 mx-auto md:mx-0 rounded-full flex items-center justify-center font-bold text-sm mb-3 <?= $sDone ? 'bg-green-500 text-white' : ($sActive ? 'bg-hvu-red text-white' : 'bg-gray-100 text-gray-400') ?>">
                                <?= $sDone ? '<i class="fas fa-check"></i>' : $s ?>
                            </div>
                        ...
                        <!-- Step Final (Choice) -->
                        <?php $s=$totalSteps; $sDone=$isDone($s); $sActive=($nextStep==$s); ?>
                        <div class="group relative bg-white p-4 rounded-2xl border-2 <?= $sActive ? 'border-hvu-red shadow-lg shadow-red-100' : ($sDone ? 'border-green-500' : 'border-gray-100') ?> transition hover:-translate-y-1 text-center md:text-left">
                             <div class="w-10 h-10 mx-auto md:mx-0 rounded-full flex items-center justify-center font-bold text-sm mb-3 <?= $sDone ? 'bg-green-500 text-white' : ($sActive ? 'bg-hvu-red text-white' : 'bg-gray-100 text-gray-400') ?>">
                                <?= $sDone ? '<i class="fas fa-check"></i>' : $s ?>
                            </div>
    ...
    <?php else: ?>
        ...
        <div class="mb-20">
            ...
            <style>
                .news-slider { scrollbar-width: none; -ms-overflow-style: none; }
