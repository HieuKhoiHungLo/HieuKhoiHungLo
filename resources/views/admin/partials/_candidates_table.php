            ...
            <thead class="bg-gray-50 text-[11px] uppercase font-bold text-slate-500">
                ...
                <tr class="bg-gray-50/50">
                    ...
                    <th class="sticky-col sticky-col-trangthai bg-white px-2 py-1 relative">
                        <select id="status_filter_select" data-filter-key="status" aria-label="Lọc trạng thái" onchange="window.applyCandidateFilters()" class="w-full text-[9px] border border-slate-200 rounded px-1 py-1 outline-none focus:border-blue-400 bg-white">
...
<script>
    ...
    function confirmSendEmail() {
        ...
        const sendToAllEl = document.getElementById('email-modal-send-to-all');
        const isSendToAll = sendToAllEl && sendToAllEl.value === 'true';

        if (isSendToAll) {
            const currentSessionId = document.getElementById('email-modal-current-session-id').value;
            submitBulk('send_email', {
                ...
                'ids[]': 'ALL' // Pass dummy value to satisfy the ids empty check in controller
            });
        } else {
            submitBulk('send_email', {
                'template_id': templateId,
