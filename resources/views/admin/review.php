                ...
                <?php if(!empty($path)): ?>
                    ...
                    <div class="relative w-full">
                        <img id="<?= $imgId ?>" loading="lazy" src="<?= $src ?>" 
                             ondblclick="window.open('<?= $link ?>', '_blank')" 
                             class="w-full h-auto object-contain cursor-pointer transition-transform duration-300 group-hover:scale-[1.3] group-hover:z-50 relative block rounded-xl border-2 border-slate-200 shadow-sm bg-slate-50" 
...
function toggleEdit(section) {
    ...
    if (section === 'personal' && !editDiv.classList.contains('hidden')) {
        // AlpineJS component handles ward loading automatically via x-data init
    }

    if (section === 'academic' && !editDiv.classList.contains('hidden')) {
        const provinceSchool = document.querySelector('#form_academic select[name="ma_tinh_lop_12"]');
        const schoolSelect = document.getElementById('ma_truong_lop_12_academic');
        if (provinceSchool && schoolSelect && schoolSelect.options.length <= 1) {
             loadSchools(provinceSchool.value, 'ma_truong_lop_12_academic', schoolSelect.getAttribute('data-selected'));
        }
    }
}
...
function previewImage(input, imgId) {
    if (input.files && input.files[0]) {
        if (imgId === 'img_ev_kv') { switchEvidenceGroup('priority'); switchSubTab('kv'); }
        else if (imgId === 'img_ev_dt') { switchEvidenceGroup('priority'); switchSubTab('dt'); }
        else if (imgId === 'img_ev_10') { switchEvidenceGroup('academic'); switchSubTab('10'); }
        else if (imgId === 'img_ev_11') { switchEvidenceGroup('academic'); switchSubTab('11'); }
        else if (imgId === 'img_ev_12') { switchEvidenceGroup('academic'); switchSubTab('12'); }
