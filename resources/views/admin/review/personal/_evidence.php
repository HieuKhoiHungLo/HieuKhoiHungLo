    ...
    <div class="w-1/3 mx-auto">
        ...
        <?php else: ?>
            <div class="relative group">
                ...
                ?>
                <img id="current_avatar" loading="lazy" src="<?= $avatarSrc ?>" ondblclick="window.open('<?= strpos($user['anh_dai_dien'], 'http') === 0 ? $user['anh_dai_dien'] : asset($user['anh_dai_dien']) ?>', '_blank')" class="w-full aspect-[2/3] object-cover rounded-xl shadow-md border-2 border-slate-200 transition-transform duration-300 group-hover:scale-[1.3] group-hover:z-50 relative cursor-pointer" title="Double click to view full size">
                <a href="<?= strpos($user['anh_dai_dien'], 'http') === 0 ? $user['anh_dai_dien'] : asset($user['anh_dai_dien']) ?>" target="_blank" class="absolute inset-0 z-[60] flex items-center justify-center opacity-0 group-hover:opacity-100 bg-white/30 rounded-xl transition"><i class="fas fa-external-link-alt text-2xl text-slate-800"></i></a>
                <button type="button" onclick="rotateEvidenceImage('<?= $user['anh_dai_dien'] ?>', 'current_avatar', this)" class="absolute top-2 right-2 z-[70] w-8 h-8 bg-white/90 hover:bg-[#0066FF] text-slate-700 hover:text-white rounded-full shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform hover:scale-110" title="Xoay ảnh 90 độ">
    ...
    <div class="mt-2">
        <?php if (!empty($user['anh_cccd_truoc'])): ?>
            <div class="relative group mb-3">
                ...
                <a href="<?= strpos($user['anh_cccd_truoc'], 'http') === 0 ? $user['anh_cccd_truoc'] : asset($user['anh_cccd_truoc']) ?>" target="_blank" class="absolute inset-0 z-[60] flex items-center justify-center opacity-0 group-hover:opacity-100 bg-white/30 rounded-xl transition"><i class="fas fa-external-link-alt text-2xl text-slate-800"></i></a>
                <button type="button" onclick="rotateEvidenceImage('<?= $user['anh_cccd_truoc'] ?>', 'current_cccd_front', this)" class="absolute top-2 right-2 z-[70] w-8 h-8 bg-white/90 hover:bg-[#0066FF] text-slate-700 hover:text-white rounded-full shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform hover:scale-110" title="Xoay ảnh 90 độ">
                    <i class="fas fa-redo-alt text-sm"></i>
                </button>
    ...
    <div class="mt-2">
        <?php if (!empty($user['anh_cccd_sau'])): ?>
            <div class="relative group mb-3">
                ...
                <a href="<?= strpos($user['anh_cccd_sau'], 'http') === 0 ? $user['anh_cccd_sau'] : asset($user['anh_cccd_sau']) ?>" target="_blank" class="absolute inset-0 z-[60] flex items-center justify-center opacity-0 group-hover:opacity-100 bg-white/30 rounded-xl transition"><i class="fas fa-external-link-alt text-2xl text-slate-800"></i></a>
                <button type="button" onclick="rotateEvidenceImage('<?= $user['anh_cccd_sau'] ?>', 'current_cccd_back', this)" class="absolute top-2 right-2 z-[70] w-8 h-8 bg-white/90 hover:bg-[#0066FF] text-slate-700 hover:text-white rounded-full shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform hover:scale-110" title="Xoay ảnh 90 độ">
                    <i class="fas fa-redo-alt text-sm"></i>
                </button>
