...
class MasterDataRepository
{
    ...
    public function getActiveMajorsWithCombinationsBySession($sessionId)
    {
        $cacheKey = 'active_majors_with_combinations_session_' . ($sessionId ?: '0');
        return Cache::remember($cacheKey, 60, function () use ($sessionId) {
            $majors = [];
            try {
                $db = Database::getInstance()->getConnection();

                // Kiểm tra xem đợt này đã có cấu hình trong admission_benchmarks chưa
                $stmtCheck = $db->prepare("SELECT 1 FROM admission_benchmarks WHERE session_id = ? LIMIT 1");
                $stmtCheck->execute([$sessionId]);
                $hasSavedConfig = (bool)$stmtCheck->fetch();

                if ($hasSavedConfig) {
                    $stmt = $db->prepare("
                        SELECT n.* 
                        FROM dm_nganh n 
                        JOIN admission_benchmarks ab ON n.ma_nganh = ab.ma_nganh 
                        WHERE ab.session_id = ? AND (ab.kich_hoat = true OR ab.kich_hoat = '1')
                    ...
                    $stmt = $db->query("
                        SELECT * 
                        FROM dm_nganh 
                        WHERE COALESCE(kich_hoat, true) = true 
                        ORDER BY ma_nganh ASC
                    ");
                }
