...
try {
    ...
    echo "<h1>Cập nhật Menu 'Số liệu trúng tuyển'</h1>\n";
    ...
    // 2. Kiểm tra & Cập nhật menu "Số liệu trúng tuyển" (/admin/admission/overview-results)
    $checkMenu = $db->prepare("SELECT id FROM menus WHERE url = '/admin/admission/overview-results' LIMIT 1");
    $checkMenu->execute();
    ...
    if ($menu) {
        ...
        echo "Đã cập nhật Menu 'Số liệu trúng tuyển' (ID: {$menu['id']}) thuộc nhóm 'TỔNG QUAN'\n";
    } else {
        $insMenu = $db->prepare("INSERT INTO menus (parent_id, title, url, icon, permission_required, order_index, position, is_active) VALUES (?, 'Số liệu trúng tuyển', '/admin/admission/overview-results', 'fa-chart-bar', 'stats', 40, 'admin_sidebar', TRUE)");
        $insMenu->execute([$groupId]);
        echo "Đã tạo mới Menu 'Số liệu trúng tuyển' thuộc nhóm 'TỔNG QUAN'\n";
    }
